...
async def test_runtime_registers_default_agents_and_handles_message(tmp_path: Path) -> None:
    ...
    assert {"general", "k8s", "workflow", "patrol", "guide", "game_audit"}.issubset(set(runtime.registry.agent_names))
...
def test_runtime_from_config_applies_storage_persistence_config(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir(parents=True, exist_ok=True)
    config = Config()
    config.storage.retention_days = 15
